...
def main() -> int:
    ...
    features = extractor.extract_video(str(video_path), frame_indices=list(range(total_frames)))
    if len(features) != total_frames:
        raise RuntimeError(f"Feature count mismatch: got {len(features)} for {total_frames} frames")
    ...
    pca_rgb = pca_rgb.reshape(total_frames, grid_side, grid_side, 3)

    cls_sim = _project_cosine_cls(patch_tokens, cls_tokens).reshape(total_frames, grid_side, grid_side)
    ...
    try:
        for frame_idx in range(total_frames):
            ...
            cv2.putText(rgb_bgr, f"frame={frame_idx:04d} t={frame_idx / fps:05.2f}s", (12, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)

            panels = [rgb_bgr, feat_bgr]
            if args.panel_mode == "rgb_pca_cls":
                sim = (cls_sim[frame_idx] * 255.0).round().astype(np.uint8)
