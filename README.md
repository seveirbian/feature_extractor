...
| 分支 | 内容 | 模型 |
|------|------|------|
| `dino` | 逐帧 DINOv3 patch token 特征(视觉表征) | DINOv3(默认 `dinov3_vits16`) |
...
|---|---|---|---|
| `dino/features` | float32 | `(T, N+1, D)` | patch token 特征;含 1 个 CLS + N 个 patch。`dinov3_vits16` 下 `D=384`(默认配置 `N+1=1025`)。退化为全局描述子时为 `(T, D)` |
...
选择 DINO 权重(默认 `dinov3_vits16`,加 `--dino_model` 切换;详见下方「DINO backbone」):
...
| `--dino_model` | `dinov3_vits16` | DINO 模型(见下方可选 backbone) |
...
|----------------|------|-------------------------|
| `dinov3_vits16`(默认) | DINOv3 ViT-S/16,embed_dim 384 | `third_party/dinov3/checkpoints/dinov3-vits16-hf/` |
| `dinov3_vits16plus` | DINOv3 ViT-S+/16,embed_dim 384 | `third_party/dinov3/checkpoints/dinov3-vits16plus-hf/` |
...
dino = DINOExtractor(model_name="dinov3_vits16", device="cuda")   # 默认;或 dinov3_vits16plus
...
dino = DINOExtractor(model_name="dinov3_vits16", device="cuda")
...
- **全帧流式支持 DINO / Depth / Pose 三个分支**。Pose 为 VGGT 滑窗 + sim(3) 拼接,超长视频上
  跨窗会有缓慢漂移,增大 `--pose_overlap` 可减小;平移仍为归一化尺度、非米制。见第 6.1 节。
